...
import type { initialPostData } from "@/app/maintimeline/_conponents/PostContainerComponsnts/postCompoenets/MenuPostOption/MenuPostOption"
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button
            variant="ghost"
            className={cn(
              "w-max justify-start p-2 min-h-max  ",
          ...
          >
            {isEditing && fullSelected ? (
              <p className="text-white">
                <span>
                  {fullSelected.flag} {fullSelected.country} ,{fullSelected.city}.
                </span>
              </p>
            ) : (
              <MapPin className={cn("h-5 w-5  ", activeLocation && " fill-white text-emerald-500 ")} />
            )}
          </Button>
        </DialogTrigger>
        <DialogContent
        ...
        >
          ...
          <div className="">
            <form onSubmit={handleSearch} className="w-80 sticky top-0  space-y-4">
              ...
              <div className="flex w-full justify-start items-start gap-3">
                <Button type="button" disabled className="w-full flex justify-center items-center gap-1">
          ...
          {locations && (
            <AnimatePresence>
              ...
              >
                {isLoading || isFetching ? (
                  <div className="ml-4 gap-4 flex justify-start items-start flex-col">
                    {Array.from({
                      ...
                      return <Skeleton key={i} className="w-[20rem] rounded-md h-2 bg-slate-100" />
                    })}
                  </div>
                ) : error ? (
                  ...
                  <div className="bg-card md:w-max max-w-max relative  text-card-foreground rounded-md shadow-md md:p-4 p-0">
                    {fullSelected && (
                      <li
                        key={fullSelected.id}
                        onClick={() => handleClick(fullSelected.id, fullSelected)}
                      ...
                      >
                        ...
                        >
                          <span>
                            {fullSelected.flag} {fullSelected.country} , {fullSelected.street}, {fullSelected.city},{" "}
                        ...
                        {locations &&
                          locations.map((location: any) => (
                            ...
                            >
                              ...
                              >
                                <span>
                                  {location.flag} {location.country} , {location.street}, {location.city},{" "}
                ...
                {!locations && <p className="text-sm text-muted-foreground">Enter location details to search.</p>}
              </motion.div>
            </AnimatePresence>
          )}
        </DialogContent>
