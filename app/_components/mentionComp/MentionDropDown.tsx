...
import { setMentionPagination } from "@/store/Reducers/pagganitionSlice";
...
setInputValue
  ...
  const handleSelectSuggestion = (item: ShapeOfUserSearchMention | string) => {
    const selectedValue =
 ...
 useEffect(() => {
  ...
  const fetchMoreData = useCallback(() => {
    if (!isFetching && users?.hasMore) {
      dispatch(setMentionPagination({ skip: skip + take, take }));
    }
