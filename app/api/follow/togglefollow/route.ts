...
const body = await req.json() as  togglefollowInfer
  ...
  if (!isAlreadyFollowing && state === "follow") {
    ...
    return NextResponse.json({ state: "followed"  ,id :follow.id }, { status: 200 });
   ...
   // data: { followingId: 3, followerId: 6 },
    ...
    return NextResponse.json({ state: "following" , id : {data: {
      followingId: main_user_id,
      followerId: author_user_id,
    },
    follow,
    ...
    return NextResponse.json({ state: "unfollow" ,  id :follow.id}, { status: 200 });
  }
  else {
