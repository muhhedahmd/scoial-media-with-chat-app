          ...
          {uniqe?.map((react, i) => {
            return (
              <div key={i} className="flex justify-start items-center w-max">
                <p className={`-rotate-3 ${getColor(react)}`}>{getEmoji(react)}</p>
              </div>
            )
          })}
          <p className="text-muted-foreground">{data.length ? data.length : null}</p>
        </div>
      </DialogTrigger>
        ...
        <div className="w-full h-full flex justify-start flex-col gap-4 items-start ">
          {data?.map((react) => {
            if (react && react.author_id)
              return (
                <ReactViewrDialogSingle userId={userId} MainUserProfile={MainUserProfile} key={react.id} {...react} />
              )
          })}
        </div>
      </DialogContent>
