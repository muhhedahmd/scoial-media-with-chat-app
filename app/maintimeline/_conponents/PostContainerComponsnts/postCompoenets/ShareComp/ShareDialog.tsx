...
const ShareDialog = ({
  ...
  const { data: MainUserProfileId, isLoading: status } = useGetProfileQuery({ userId: user?.id })
    ...
    <Dialog open={openDialog} onOpenChange={setOpenDialog}>
      ...
      <DialogContent className="max-w-[25rem] rounded-md sm:max-w-[40rem]">
        <div className="flex flex-col justify-start items-start ">
          ...
          />
          <MinmalCard author_id={author_id} postId={postId} user={user} title={title} />
        </div>

        <DialogFooter className="flex justify-start flex-col sm:flex-row w-full gap-2 ">
          <DialogClose disabled={isLoading}>
            {isLoading ? <LoaderCircle className="animate-spin text-gray-600 w-4 h-4" /> : "Later"}
          </DialogClose>
          <Button disabled={isLoading} onClick={() => handleShare()}>
            {isLoading ? <LoaderCircle className="animate-spin text-gray-600 w-4 h-4" /> : "Share"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
